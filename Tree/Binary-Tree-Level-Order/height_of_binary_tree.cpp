...
int max_height_of_binary_tree(Node* root){
    if(root==NULL)
        return 0;
    if(root->left==NULL && root->right==NULL)
        return 0;
    
    int l=max_height_of_binary_tree(root->left);
    int r=max_height_of_binary_tree(root->right);
    return max(l,r)+1 ;  
}
...
int main()
{
  ...
  cout<<max_height_of_binary_tree(root);
